...
<template>
  <div class="portfolio-report">
    <header class="report-header">
      ...
      <section class="report-section" v-if="porfolioReport">
        <h3 class="section-title">Métricas Principales</h3>
        <div class="kpi-grid">
          <div class="kpi-card highlight">
            <div class="kpi-label">Total Prestado</div>
            <div class="kpi-value gold">{{ formatCurrency(porfolioReport?.totalPrestado) }}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Capital Pendiente</div>
            <div class="kpi-value">{{ formatCurrency(porfolioReport?.capitalPendiente) }}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Intereses Generados</div>
            <div class="kpi-value">{{ formatCurrency(porfolioReport?.interesesGenerados) }}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Intereses Cobrados</div>
            <div class="kpi-value success">{{ formatCurrency(porfolioReport?.interesesCobrados) }}</div>
          </div>
        </div>
      </section>
      <section class="report-section" v-if="porfolioReport">
        ...
        <div class="kpi-grid">
          <div class="kpi-card">
            ...
            <div class="kpi-value">{{ porfolioReport?.numeroPrestamosActivos }}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Total Clientes</div>
            <div class="kpi-value">{{ porfolioReport?.totalClientes }}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Tasa Interés Promedio</div>
            <div class="kpi-value">{{ porfolioReport?.tasaInteresPromedio }}%</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">% Recuperado</div>
            <div class="kpi-value success">
              {{ porfolioReport.totalPrestado > 0 ? ((1 - porfolioReport.capitalPendiente / porfolioReport.totalPrestado) * 100).toFixed(1) : 0 }}%
            </div>
          </div>
        </div>
      </section>
