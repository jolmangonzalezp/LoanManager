...
const capitalFormatted = computed({
    get() {
        return paymentForm.value?.amount
            ? paymentForm.value.amount.toLocaleString('es-CO')
            : ''
    },
    ...
    set(value: string) {
        ...
        paymentForm.value.amount = Number(
...
const save = async () => {
    if (!paymentForm.value) return
    if (props.isEditing) {
        const response = await update(props.id, paymentForm.value);
        if (response) {
            close();
        }
    }else {
        const response = await create(paymentForm.value)
        if (response) {
            close();
        }
    }
