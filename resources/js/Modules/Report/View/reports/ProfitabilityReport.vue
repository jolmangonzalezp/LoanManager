...
import { useReports } from '@/Modules/Report/index.ts';

const { columnsProfitable, loanProfitable, profitabilityReport, getProfitability } = useReports();
...
<template>
  <div class="profitability-report">
    <header class="report-header">
      ...
      <section class="report-section" v-if="profitabilityReport">
        <h3 class="section-title">Resumen</h3>
        <div class="kpi-grid">
          <div class="kpi-card highlight">
            <div class="kpi-label">Total Intereses</div>
            <div class="kpi-value gold">{{ formatCurrency(profitabilityReport.totalIntereses) }}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Total Capital</div>
            <div class="kpi-value">{{ formatCurrency(profitabilityReport.totalCapital) }}</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">Ratio Intereses/Capital</div>
            <div class="kpi-value">{{ (profitabilityReport.ratioInteresesCapital * 100).toFixed(2) }}%</div>
          </div>
          <div class="kpi-card">
            <div class="kpi-label">ROI Global (mensual)</div>
            <div class="kpi-value success">{{ profitabilityReport.roiGlobal.toFixed(2) }}</div>
          </div>
        </div>
      </section>
